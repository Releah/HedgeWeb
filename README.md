...
The Web Browser application streams a pinned Chromium worker through HedgeWeb. Web traffic originates inside the Docker network and shares the VPN agent's network namespace, so approved VPN routes are available while the tunnel is connected. The worker has no published port, host directories, Docker socket, sharing, command channel, microphone, gamepad or file-transfer feature. Its profile is held in a RAM-backed `/config` mount and is discarded whenever the browser container is recreated.

The browser is one shared worker per HedgeWeb deployment in this release. Use a single trusted user at a time; per-user disposable workers remain planned. HedgeWeb proxies the worker only for authenticated sessions, but production deployments must still keep the normal Traefik and Authentik protections in front of HedgeWeb.
