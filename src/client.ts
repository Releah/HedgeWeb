...
type VpnStatus = { configured: boolean; state: 'not_configured' | 'disconnected' | 'authenticating' | 'connecting' | 'connected' | 'error'; identity: string; endpoint: string | null; address: string | null; accessScope: string; connectedAt: string | null; expiresAt: string | null; bytesReceived: number; bytesSent: number; error?: string };
type VpnConfigView = { configured: boolean; endpointLabel?: string; accessScope?: string; leaseMinutes?: number; updatedAt?: string };
type AppId = 'ssh' | 'browser' | 'settings' | 'about';
...
let profiles: Profile[] = [], identity: Identity | null = null, authMode = '', pendingProfile: Profile | null = null, pendingPassword = '', topZ = 20, terminalSequence = 0;
...
function setupSettings(win: ManagedWindow) {
  ...
  form.onsubmit = async event => { event.preventDefault(); qs('#vpn-config-error', win.element).textContent = ''; try { const values = Object.fromEntries(new FormData(form)); if (!values.profile) throw new Error('Paste a replacement OpenVPN profile before saving'); await request('/api/vpn/config', { method: 'PUT', body: JSON.stringify(values) }); (form.elements.namedItem('profile') as HTMLTextAreaElement).value = ''; await loadVpnConfig(win); await refreshVpnStatus(); } catch (error) { qs('#vpn-config-error', win.element).textContent = (error as Error).message; } };
...
function setupBrowser(win: ManagedWindow) {
  ...
  const navigate = (value: string) => { try { const url = normalizeAddress(value); history.splice(position + 1); history.push(url); position = history.length - 1; render(); window.open(url, '_blank', 'noopener,noreferrer'); } catch (error) { address.setCustomValidity((error as Error).message); address.reportValidity(); } };
