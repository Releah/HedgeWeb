...
type VpnConfig = { endpointLabel: string; accessScope: string; leaseMinutes: number; certificateOnly: boolean; encryptedProfile: string; updatedAt: string };
...
async function api(req: IncomingMessage, res: ServerResponse, pathname: string) {
  ...
  if (pathname === '/api/vpn/status' && req.method === 'GET') {
    ...
    try { return json(res, 200, { configured: true, authenticationRequired: !store.vpn.certificateOnly, ...(await vpnAgent<Record<string, unknown>>('/status')) }); } catch (error) { return json(res, 200, { configured: true, authenticationRequired: !store.vpn.certificateOnly, state: 'error', identity: actor.username, endpoint: store.vpn.endpointLabel, address: null, accessScope: store.vpn.accessScope, connectedAt: null, expiresAt: null, bytesReceived: 0, bytesSent: 0, error: (error as Error).message }); }
  }
  if (pathname === '/api/vpn/config' && req.method === 'GET') return json(res, 200, store.vpn ? { configured: true, endpointLabel: store.vpn.endpointLabel, accessScope: store.vpn.accessScope, leaseMinutes: store.vpn.leaseMinutes, certificateOnly: store.vpn.certificateOnly, updatedAt: store.vpn.updatedAt } : { configured: false });
  if (pathname === '/api/vpn/config' && req.method === 'PUT') {
    if (!actor.admin) return json(res, 403, { error: 'Administrator access required' });
    const input = await body(req); const profile = String(input.profile || ''); const endpointLabel = String(input.endpointLabel || '').trim().slice(0, 120); const accessScope = String(input.accessScope || '').trim().slice(0, 180); const leaseMinutes = Number(input.leaseMinutes || 60); const certificateOnly = input.certificateOnly === true || input.certificateOnly === 'on';
    const replacingProfile = profile.trim().length > 0;
    if (!endpointLabel || !accessScope || (!replacingProfile && !store.vpn)) return json(res, 400, { error: 'A valid OpenVPN client profile, endpoint name and access scope are required' });
    if (replacingProfile && (profile.length < 20 || profile.length > 256 * 1024 || !/^\s*client\b/im.test(profile) || !/^\s*remote\b/im.test(profile))) return json(res, 400, { error: 'The OpenVPN client profile is invalid' });
    if (replacingProfile && (/^\s*(up|down|route-up|ipchange|tls-verify|auth-user-pass-verify|client-connect|client-disconnect|learn-address|plugin|management|management-client|daemon|log|log-append|status|writepid|chroot|cd|setenv|setenv-safe|askpass|pkcs11-providers|pkcs11-id|script-security)\b/im.test(profile) || /<auth-user-pass>/i.test(profile))) return json(res, 400, { error: 'The profile contains a prohibited executable, credential or management directive' });
    ...
    store.vpn = { endpointLabel, accessScope, leaseMinutes, certificateOnly, encryptedProfile: replacingProfile ? encryptVpnProfile(profile) : store.vpn!.encryptedProfile, updatedAt: new Date().toISOString() }; saveStore(); return json(res, 200, { configured: true, endpointLabel, accessScope, leaseMinutes, certificateOnly, updatedAt: store.vpn.updatedAt });
  }
  if (pathname === '/api/vpn/connect' && req.method === 'POST') {
    ...
    if (!store.vpn.certificateOnly && (!username || !password || (otp && !/^\d{6,8}$/.test(otp)))) return json(res, 400, { error: 'Enter a username, password and a valid authenticator code' });
    const result = await vpnAgent('/connect', { method: 'POST', body: JSON.stringify({ profile: decryptVpnProfile(store.vpn.encryptedProfile), username, password, otp, certificateOnly: store.vpn.certificateOnly, identity: actor.username, endpoint: store.vpn.endpointLabel, accessScope: store.vpn.accessScope, leaseMinutes: store.vpn.leaseMinutes }) }); return json(res, 202, result);
  }
